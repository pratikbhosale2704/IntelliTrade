...
async function fetchCreditDetails() {
    ...
    try {
        credit = await contract.getCreditDetails(creditId);
      ...
      <strong>Expiry Date:</strong> ${new Date(Number(credit[5]) * 1000).toLocaleDateString()} <br>
...
function setupUI() {
    document.getElementById("buyQuantity").addEventListener("input", function () {
        ...
        document.getElementById("totalPrice").value = (quantity * pricePer).toFixed(6);
    });
...
async function executePurchase() {
    ...
    if (!contract || typeof contract.BuyCredit !== "function") {
    ...
    try {
        ...
        const tx = await contract.BuyCredit(quantity, creditId, {
            value: totalValue,
