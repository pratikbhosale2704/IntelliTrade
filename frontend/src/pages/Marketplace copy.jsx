// frontend/src/pages/Marketplace.jsx :-> before AI
...
const Marketplace = () => {
  ...
  // Fetch assets from the blockchain and database
      ...
      const assets = await Promise.all(
        assetIds.map(async (id) => {
          try {
            ...
            if (isForSale) {
              // Fetch title and description from the database
              ...
              return {
                id: assetId.toString(),
                owner: owner.toLowerCase(), // Store the owner's address
                title: title || `Asset ${assetId}`,
                description: description || `No description available for asset ${assetId}.`,
                assetType,
                price: (price.toString() / 10 ** 18).toFixed(4), // Convert wei to ETH
                aiValuation: (price.toString() * 1.1 / 10 ** 18).toFixed(4), // Example AI valuation
    ...
    const applyFilters = () => {
      ...
      if (filters.type) {
        filtered = filtered.filter(
          (asset) => asset.assetType.toLowerCase() === filters.type.toLowerCase()
        );
      }
      ...
      if (filters.minPrice) {
        filtered = filtered.filter(
          (asset) => asset.price >= parseFloat(filters.minPrice)
        );
      }
      ...
      if (filters.maxPrice) {
        filtered = filtered.filter(
          (asset) => asset.price <= parseFloat(filters.maxPrice)
        );
      }
