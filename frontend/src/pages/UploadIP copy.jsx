...
const UploadIP = () => {
  ...
  //     if (!response.ok) {
  //       throw new Error("Failed to store asset in the database.");
  //     }
  ...
  //     // Store the asset data in MongoDB
  //     await storeAssetInDB({ assetID: latestAssetID.toString(), title, description });
  ...
  const mint = async () => {
    ...
    try {
      ...
      await storeAssetInDB({
        ...
        title,
        description,
        aiValuation: aiData.estimated_price || 1.1 * price, // Fallback to 1.1 * price if AI valuation is not available
      });
